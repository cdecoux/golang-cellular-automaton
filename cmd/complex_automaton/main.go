...
func main() {
	...
	customRule := func(self automaton.ComplexAutomaton2D, x, y int) int {
		return rand.Intn(len(self.GetTypes()) + 1)
	}

	gridAutomaton := automaton.NewComplexAutomaton2D(width, height, []string{"A", "B", "C", "D", "E"}, customRule)
	gridAutomaton.FillRandom()


	grid := ui.NewComplexGridDisplay(gridAutomaton, width, height)
